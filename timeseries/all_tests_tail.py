...
# Lab11 tests...
class TestTSMeanMedian(unittest.TestCase):
  ...
  def testMedian1(self):
    t = TimeSeries([1], [-10])
    self.assertEqual(t.median(), -10)
    scores.append( ('#ar Lab11', 'median base', 1))
  def testMedian2(self):
    t = TimeSeries([1, 2], [1, 2])
    self.assertEqual(t.median(), 1.5)
    scores.append( ('#ar Lab11', 'median base', 1))
  def testMedian3(self):
    t = TimeSeries([1, 2, 2.1], [3, 1.5, 1])
    self.assertEqual(t.median(), 1.5)

    t = TimeSeries([1, 2, 3, 4], [4, 3, 2, 1])
    self.assertEqual(t.median(), np.median(np.array([1, 2, 3, 4])))
    scores.append( ('#ar Lab11', 'median base', 1))
  def testMedian4(self):
    ...
    with self.assertRaises(Exception):
      t = TimeSeries([],[])
      t.median()
    scores.append( ('#ar Lab11', 'median base', 2))
...
class TestTSUnaryyOperators(unittest.TestCase):
  ...
  def testOps3(self):
    t_a = TimeSeries([1, 2, 3, 4], [0.1, 0.2, 0.3, 0.4])
    t_b = TimeSeries([1, 2, 3, 4], np.array([-0.1, -0.2, -0.3, -0.4]))  
    self.assertTrue(bool(t_a))
    scores.append( ('#ar Lab15', 'bool', 1))

  def testOps4(self):
    t_a = TimeSeries([1, 2], [20., -9.])
    self.assertEqual(abs(t_a), np.sqrt([20. * 20. + 9 * 9]))
...
class TestTSBinaryOperators(unittest.TestCase):
  ...
  def testAdd1(self):
    t_a = TimeSeries([1, 2, 3, 4], [0.1, 0.2, 0.3, 0.4])
    t_b = TimeSeries([1, 2, 3, 4], np.array([0.1, 0.2, 0.3, 0.4]))
    t_c = TimeSeries([1, 2, 3, 4], [0.2, 0.4, 0.6, 0.8])
    t_d = TimeSeries([1, 2, 3, 4], [1.1, 1.2, 1.3, 1.4])
    t_e = TimeSeries([1, 2, 3], [0.5, 0.2, 0.9])
    t_f = TimeSeries([], [])

    self.assertTrue( (t_a + t_b) == t_c)

    scores.append( ('#ar Lab15', 'add base', 1))
  ...
  def testAdd2(self):
    ...
    scores.append( ('#ar Lab15', 'add adv', 3))
  ...
  def testAdd3(self):
    ...
    scores.append( ('#ar Lab15', 'add not impl', 1))
  ...
  def testAdd4(self):
    ...
    scores.append( ('#ar Lab15', 'add + int val', 2))
  ...
  def testAdd6(self):
    ...
    scores.append( ('#ar Lab15', 'add + float val', 1))
  ...
  def testAdd7(self):
    ...
    scores.append( ('#ar Lab15', 'add + val err', 2))
  ...
  def testSub2(self):
    ...
    scores.append( ('#ar Lab15', 'sub adv', 3))
  ...
  def testMul1(self):
    t_a = TimeSeries([1, 2, 3, 4], [0.1, 0.2, 0.3, 0.4])
    t_b = TimeSeries([1, 2, 3, 4], np.array([2., 3., 3., 2.]))
    t_c = TimeSeries([1, 2, 3, 4], [0.2, 0.6, 0.9, 0.8])
...
class TestTSDocs(unittest.TestCase):

  def test_all_docstrings(self):
    REQUIRED_DOCSTRINGS = [ '__init__', '__str__', 'items', '__contains__', '__iter__', '__eq__', \
    '__add__', '__sub__', '__mul__', 'itertimes', 'itervalues', 'iteritems', '__abs__', '__bool__']

    t = TimeSeries([1, 2, 3], [1,2,3])

    for req in REQUIRED_DOCSTRINGS:
      try:
        if len(getattr(t,req).__doc__ )>10:
          scores.append( ('#dc General', 'item docstrings: '+req, 1))
      except:
        pass
      #self.assertIsNotNone( getattr(t,req), '__doc__' )
      # 10 characters is not sufficient. This is to weed out empty docstrings.
      #self.assertTrue( len(getattr(t,req).__doc__ )>10 )

    #scores.append( ('#dc General', 'item docstrings', 5))

  def test_class_docstring(self):
    t = TimeSeries([1, 2, 3], [1,2,3])
    # Also make sure the class has one.
    self.assertIsNotNone(t.__doc__)
    self.assertTrue( len(t.__doc__ )>10 )

    scores.append( ('#dc General', 'class docstrings', 3))